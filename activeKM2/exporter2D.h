...
public:
  ...
  template <typename TPar>
  void get_data_from_par(const std::vector<TPar>& p_arr, float* pos, float* psi);
...
template<typename TPar>
void exporter::Snap_GSD_2::get_data_from_par(const std::vector<TPar>& p_arr, float* pos, float* psi) {
  size_t n_par = p_arr.size();
  for (size_t j = 0; j < n_par; j++) {
    size_t j3 = j * 3;
    pos[j3] = p_arr[j].pos.x - half_l_.x;
    pos[j3 + 1] = p_arr[j].pos.y - half_l_.y;
    pos[j3 + 2] = p_arr[j].get_theta();
    psi[j] = p_arr[j].get_psi();
  }
}


template<typename TPar>
void exporter::Snap_GSD_2::get_data_from_par(const std::vector<TPar>& p_arr, float* pos, float* psi, float* omega) {
  size_t n_par = p_arr.size();
  for (size_t j = 0; j < n_par; j++) {
    size_t j3 = j * 3;
    pos[j3] = p_arr[j].pos.x - half_l_.x;
...
void Snap_GSD_2::read(int i_frame, std::vector<TPar>& p_arr) {
  ...
  gsd_read_chunk(handle_, psi, chunk);
