...
def adjust_density(s, phi_A: float, phi_B: float, mode="copy", R=5):
...
def add_A_particles(s, nA_added):
    Lx = int(s.configuration.box[0])
    Ly = int(s.configuration.box[1])
...
def shift_v_slice(s, x1: float, x2: float, dy: float):
    Ly = s.configuration.box[1]
    x = s.particles.position[:, 0]
...
def shift_h_slice(s, y1: float, y2: float, dx: float):
    Lx = s.configuration.box[0]
    x = s.particles.position[:, 0]
    y = s.particles.position[:, 1]
...
def shift_pos(s, dx: float, dy: float):
    Ly = s.configuration.box[1]
    Lx = s.configuration.box[0]
...
def swap_h(s, x1: float, x2: float, dx: float):
    Lx = s.configuration.box[0]
    x = s.particles.position[:, 0]
